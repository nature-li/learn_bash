...
start() {
    if [ ${CUR_USER} = ${RUN_USER} ]; then
        ...
        su - ${RUN_USER} -c "${RAW_SERVICE} start"
    else
        # start program error
...
stop() {
    if [ ${CUR_USER} = ${RUN_USER} ]; then
        ...
        su - ${RUN_USER} -c "${RAW_SERVICE} stop"
    else
        # stop program error
        echo "[$CUR_USER] is not a permitted user, you must be [${RUN_USER}] or [${ROOT_USER}]"
    fi
...
status() {
    if [ ${CUR_USER} = ${RUN_USER} ]; then
        ...
        # stop program as root user
        su - ${RUN_USER} -c "${RAW_SERVICE} status"
    else
        # stop program error
        echo "[$CUR_USER] is not a permitted user, you must be [${RUN_USER}] or [${ROOT_USER}]"
    fi
...
debug() {
    if [ ${CUR_USER} = ${RUN_USER} ]; then
        ...
        su - ${RUN_USER} -c "${RAW_SERVICE} debug"
    else
        # debug program error
        echo "[$CUR_USER] is not a permitted user, you must be [${RUN_USER}] or [${ROOT_USER}]"
    fi
...
restart() {
    if [ ${CUR_USER} = ${RUN_USER} ]; then
        ...
        su - ${RUN_USER} -c "${RAW_SERVICE} restart"
    else
