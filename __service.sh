...
# set main class
APP_MAIN_CLASS=com.fuck.adtech.report.generator.AdTechReportGenerator

# set mock class
APP_MOCK_CLASS=com.fuck.adtech.report.counter.mock.Mock
...
check_pid() {
    ...
    if [ "empty${EXPECTED_PID}" != "empty" ]; then
        DAEMON_ID=`ps aux | grep ${APP_MAIN_CLASS} | grep -v grep | grep ${EXPECTED_PID} | awk '{print $2}'`
        if [ "${EXPECTED_PID}" != "${DAEMON_ID}" ]; then
            DAEMON_ID=0
        fi
    fi

    if [ "empty${DAEMON_ID}" == "empty" ]; then
        DAEMON_ID=0
    fi
}
...
debug() {
    ...
    if [ ${DAEMON_ID} -ne 0 ]; then
        echo "======================================================================="
        echo "warn: ${APP_MAIN_CLASS} already started! (pid=${DAEMON_ID})"
        echo "======================================================================="
        exit -1
    else
...
start() {
    ...
    if [ ${DAEMON_ID} -ne 0 ]; then
        ...
        # wait 5 seconds for starting
        for ((second=1; second<=5; second++))
        do
            sleep 1
            check_pid
            if [ ${DAEMON_ID} -ne 0 ]; then
                echo "Running $APP_MAIN_CLASS for ${second} seconds...(pid=${DAEMON_ID})"
            else
        ...
        if [ ${DAEMON_ID} -ne 0 ]; then
            echo "Starting $APP_MAIN_CLASS (pid=${DAEMON_ID}) [OK]"
        else
...
stop() {
   ...
   if [ ${DAEMON_ID} -ne 0 ]; then
        echo -n "Send signal to ${APP_MAIN_CLASS} ...(pid=${DAEMON_ID}) "
        kill -TERM ${DAEMON_ID}
        if [ $? -eq 0 ]; then
             echo "[OK]"
        else
        ...
        do
            check_pid
            if [ ${DAEMON_ID} -ne 0 ]; then
                echo "waiting ${APP_MAIN_CLASS} to stop ...(pid=${DAEMON_ID}) "
                sleep 1
            else
                break
            fi
        ...
        echo "${APP_MAIN_CLASS} is stopped...(pid=${DAEMON_ID}) "
   else
        echo "======================================================================="
...
status() {
    ...
    if [ ${DAEMON_ID} -ne 0 ];  then
        echo "${APP_MAIN_CLASS} is running! (pid=${DAEMON_ID})"
    else
