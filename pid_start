...
# http_server file name
SERVER_NAME=http_server

# api server file path
SERVER_SCRIPT=${SERVER_PATH}/${SERVER_NAME}
...
check_pid() {
    ...
    if [ "${DAEMON_PID_FROM_FILE}" != "" ]; then
        DAEMON_PID=$(ps aux | grep -v grep | grep python | grep ${SERVER_NAME} | grep ${DAEMON_PID_FROM_FILE} | awk '{print $2}')
